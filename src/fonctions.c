...
size_t initTaille(){ /* INITIALISE LA TAILLE DE LA GRILLE DE SUDOKU. */
	...
	while((taille!=4) && (taille!=9) && (taille!=16)){ /* code ascii de 1, 2, et 3 pour securiser la saisie de la taille */
	...
	printf("taille : %zu \n", taille);
		...
		if(taille==1)
			taille = 4; /* convertir le choix 49 en taille 4 */
		else if(taille==2)
			taille = 9; /* convertir le choix 50 en taille 9 */
		else if(taille==3)
			taille = 16; /* convertir le choix 51 en taille 16 */
	}
	return taille;
}
	...
	if (fichier != NULL){
		...
		for(i=0;i<taille;i++){
			for (j=0; j<taille; j++){
		  		...
		  		if(fread(&grilleSolution[i][j].valeur, sizeof(int), 1, fichier));
		  	}
		}

		for(i=0;i<taille;i++){
			for (j=0; j<taille; j++){
		  		if(fread(&grille[i][j].valeur, sizeof(int), 1, fichier));
				if(fread(&grille[i][j].modifiable, sizeof(int), 1, fichier));	
			}
		}
	
		if(fread(temps, sizeof(float), 1, fichier));
...
bool estInclus(uint16_t singleton, uint16_t possibilitesCase, size_t taille){ /* VERIFIE QUE LE SINGLETON EST PRESENT DANS LES POSSIBILITES DES AUTRES CASES DE LA LIGNE/COLONNE/REGION */
...
bool crossHatching(caseGrille *sousGrille, size_t taille){ /* CROSS-HATCHING */
...
bool loneNumber(caseGrille* sousGrille, size_t taille) { /* LONE-NUMBER */
	bool present = false;
	size_t j;
...
bool nakedSubset(caseGrille *sousGrille, size_t taille){ /* NAKED SUBSET */
...
caseGrille** convertirZoneEnTableau(caseGrille** grille, size_t ligne, size_t colonne, size_t taille) {  /* CONVERTIR UNE ZONE DE LA GRILLE EN TABLEAU */
	...
	for (ligne = i; ligne < i+sqrt(taille); ligne++) {
		for (colonne = j; colonne < j+sqrt(taille); colonne++) {
			tabZone[cpt] = &grille[i][j]; /* on recopie l'adresse des cases de la zone dans le tableau tabZone*/
			cpt++;
		}
	}
...
caseGrille** convertirColonneEnTableau(caseGrille** grille, size_t ligne, size_t colonne, size_t taille){ /* CONVERTIR UNE COLONNE DE LA GRILLE EN TABLEAU */
...
void initAide(caseGrille** grille, size_t taille) { /* INITIALISER L'AIDE */
	for (size_t i = 0; i < taille; i++) {
		for (size_t j = 0; j < taille; j++) {
			if (grille[i][j].valeur != 0) {
				grille[i][j].aide = (1u << (grille[i][j].valeur - 1)); /* s'il y a une valeur dans la case, on initialise l'aide de cette case : la seule valeur possible c'est celle qu'il y a déjà */
			}
...
void aide(caseGrille** grille, const size_t taille) { /* AIDE */	    
...
void parcoursGrilleAide(caseGrille** grille, size_t taille, size_t ligne, size_t colonne) { /* APPLIQUE LES HEURISTIQUES SUR L'ENSEMBLE DE LA GRILLE */
...
void demanderNom(char *nom){ /* ENREGISTRE LE PSEUDO */
...
void score(int temps, size_t taille){ /* SCORE */
	...
	for(size_t i=0; i<10; i++){
		if(fgets(ligne, sizeof(ligne), fichierLecture));
...
void reinitialiserTop10(size_t taille){ /* REINITIALISE LE TOP 10 */
...
void afficherTop10(size_t taille){ /* AFFICHE LE TOP 10 */
	/*On recupere le nom du fichier*/
	char* nomFichier="";
	if(taille==4)
		nomFichier="top10_4.txt";
...
void recupeGrille(caseGrille**grille, size_t taille){ /* RECUPERE UNE GRILLE ENONCE */
